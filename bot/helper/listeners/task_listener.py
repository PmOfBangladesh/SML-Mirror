 ...
 """,
    ...
    async def on_upload_complete(
        ...
        if self.is_yt:
            ...
            # ===================== FIXED OUTER CONDITION =====================
            if (
                link
                or (rclone_path and not self.private_link)
                or multi_links
            ):
                buttons = ButtonMaker()
                ...
                # ===================== MODIFIED RCLONE BUTTON BLOCK =====================
                # Only add Index Link (Rclone Link and View Link are removed)
                if rclone_path and not self.private_link:
                    # Resolve base URL: user's INDEX_URL -> Config.INDEX_URL -> Config.RCLONE_SERVE_URL (fallback)
                    index_url = ""
                    if self.private_link:
                        index_url = self.user_dict.get("INDEX_URL", "") or ""
                    elif Config.INDEX_URL:
                        index_url = Config.INDEX_URL
                    if not index_url and Config.RCLONE_SERVE_URL:
                        index_url = Config.RCLONE_SERVE_URL

                    if index_url:
                        # Remove remote name (e.g., "SML-Mirror:")
                        _, rpath = rclone_path.split(":", 1)
                        base = index_url.rstrip('/')
                        clean_path = rpath.strip('/')
                        url_path = rutils.quote(clean_path) if clean_path else ""
                        share_url = f"{base}/{url_path}" if url_path else base
                        if mime_type == "Folder" and not share_url.endswith('/'):
                            share_url += "/"

                        # Keep only Index Link button
                        buttons.url_button("⚡ Index Link", share_url)

                # Keep original GDrive index link logic (unchanged)
